...
- **Stap 6 — Trust Governance & Management Interface — GEDONE** — een operator-oppervlakte om te bepalen wát een peer over het netwerk mag: governance API (`GET /api/governance/v1/{catalog,topology,matrix}`, `POST /api/governance/v1/peers/:peerId/verify`, `PUT|DELETE /api/governance/v1/peers/:peerId/permissions`, authenticated SSE `GET /api/governance/v1/stream`) in `apps/core-server/src/governance/` + een `plugins/governance-ui`-plugin die de matrix beheert via de iframe/postMessage-bridge.
  - **Permissiematrix = intersectie (geen unie)** — `EffectiveAccess = ManifestExposed ∩ PeerMatrixAllowed ∩ VerifiedStatus`. De matrix is een **narrowing-only** store: élke skill/topic in een entry wordt bij schrijven gevalideerd tegen de live manifest-geexposeerde catalog (`AccessDeniedError` ⇒ 403) — een entry kan een skill structureel nooit openen die het manifest niet exposeert. In de broker geïnjecteerd als `PeerSkillGate` (`TaskBroker.setPeerSkillGate`); de broker-`localOnly`/`httpBridgeOnly`/`remote`-checks blijven onafhankelijk doorlopen. Matrices worden **narrowing-per-entry** toegepast: een peer zónder entry houdt de manifest-default (geen narrowing); alleen expliciet geliste peers worden ingeperkt. `isAllowed` faalt closed bij een werpende lookup; anonieme callers (geen transport-verified `peerId`) worden nooit geconsulteerd (de matrix is peerId-geadresseerd) en passeren de bestaande remote-policy.
  - **Tier-2 verplicht op alle writes** — `POST /verify` en `PUT/DELETE /permissions` (én de `governance-ui.*`-bridgeskills) lopen door `TrustTierGate.authorize("critical", …, { authenticated: true })`; een boot-token alleen is onvoldoende, een ontbrekende confirmer faalt closed (403). Matrix-persistentie atomair (`atomicWriteFile`) in `<dataDir>/governance-matrix.json`, corrupt bestand ⇒ `StorageCorruptionError` (nooit stil leeg).
  - **In-process `contacts.verifyPeer`** — governance bereikt de contacts-plugin via `host.getActivated("contacts")` (duck-typed `asContactsApi`); `verifyPeer` blijft `httpBridgeOnly` en gaat het HTTP-bridge-oppervlak dus nooit op (het manifest declareert `network:http:contacts.verifyPeer`, de skill is niet `httpExposed`).
  - **Topology = functioneel-only** — `peerId`, `displayName`, `trustState` (`none/discovered/pending/verified/blocked`, TV-verified-onderscheid: een geverifieerde identiteit zonder contactrecord leest als `pending`), `lastSeen`, `activeSubscriptions` (via nieuwe `PluginHost.listEventSubscriptions()`), per-peer matrix-entry. Nooit `address`/RTT/bandwidth (topology-leak-preventie, getest op de geserialiseerde entry).
  - **SSE `/stream`** — dezelfde boot-token-auth als alle `/api/*`-routes; `GovernanceStream` replayt de matrix-delta op heartbeat (signature-diff, live snapshot-provider), heartbeats houden EventSource open; writes komen bij elke-tier consumer aan zonder secret-side-channel.
  - **`governance-ui`-plugin = admin HTTP bridge** — alle zes skills (`getCatalog`/`getTopology`/`listPermissions`/`verifyPeer`/`registerSkills`/`removePermissions`) zijn `httpBridgeOnly: true` en worden door core-server geregistreerd (de plugin zelf registreert niets op de broker — getest); het manifest declareert `ui.skills` + matching `network:http:*`-permissies. `customRateLimit` (per-peer event-stream-cap) is bounded: `1..ABSOLUTE_MAX_RATE_LIMIT` (500), daarboven ⇒ `InvalidRateLimitError` (422), nooit "unlimited".
  - **Tests** — 4 broker-unit (narrowing, no-widen, fail-closed, anonymous-skip) + 18 governance (matrix: intersectie/rate-cap/persistentie/isAllowed/corrupt-luid; service: tier-2-deny/allow + catalog-validation + topology-functional-only; HTTP: catalog-only-exposed, 403-non-exposed, 422-over-cap, round-trip matrix, verify-fail-closed-403, topology-lege-provider, 401-zonder-token; SSE: delta-replay + heartbeat + fresh-subscriber-replay + live-provider) + 3 governance-ui-plugin. Full suite 0 fail.
- **Stap 5 — Distributed Subscriptions & Event Framing (P2P pub/sub) — GEDONE (`3211035` + `fbef052`)** — een capability-gerichte Pub/Sub-laag bovenop het bestaande `network-light`-transport, structureel klaar voor de streaming-richting van "Besluit 3" (hoogfrequente events vallen onder de TelemetryGate, niet onder de broker-taak-limiter). Geaccordeerde schikkingen: hub in **`core/src/events/`** (het spec-pad `packages/events` bestaat niet als workspace; `core` is de centrale gedeelde-laag naast hook-registry/security) en per-topic-autorisatie **uitsluitend via `manifest.exposedEvents`** (het spec-vermelde CapabilityRegistry bestaat niet; `exposedEvents` is de declaratieve source of truth — géén nieuw parallel registry-concept). De vier security-refinements van de review (per-topic-auth, TelemetryGate, publisherPeerId-provenance, wildcard-re-auth + caps) zijn hieronder verwerkt.
